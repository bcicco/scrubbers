...
export const generateLeadsForArea = async (
  productDescription,
  targetIndustry,
  area,
  productName
  ...
  try {
    ...
    const transformedLeads = data.businesses.map((business) => ({
      ...
      reasoning:
        ...
        `Potential fit for ${productDescription} in ${targetIndustry}`,
    }));

    console.log(
      `✅ Successfully transformed ${transformedLeads.length} leads for ${area}`
    );
